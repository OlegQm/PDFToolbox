...
export default function App() {
  ...
  const [rotations, setRotations] = useState([{ page: 1, degrees: 0 }]);
  ...
  const handleRotationChange = (i, field, val) => {
    setRotations(rs => rs.map((r, idx) => idx===i ? {...r, [field]: val} : r));
  };
  const addRotation = () => setRotations(rs => [...rs, { page: rs.length+1, degrees: 0 }]);
  const removeRotation = i => setRotations(rs => rs.filter((_,idx)=>idx!==i));
  const handleSubmit = async e => {
    e.preventDefault(); if (!file) return setError("Please select a PDF file.");
    setLoading(true); setError("");
    try { const blob = await rotatePdf(file, rotations); setDownloadUrl(URL.createObjectURL(blob)); }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center py-8">
      {/* Top page title */}
      <h1 className="text-5xl font-extrabold text-gray-800 mb-8">PDF Toolbox</h1>

      {/* Container with fieldset break-style border and extra-rounded corners */}
      <fieldset className="relative w-full max-w-md border-2 border-blue-500 rounded-xl bg-white p-6 overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 mt-4">
          ...
          <div>
            <h2 className="text-xl font-semibold text-gray-800 mb-4">Page Rotations</h2>
              ...
              {rotations.map((r, idx) => (
                <div key={idx} className="grid grid-cols-3 gap-4 items-end">
                  <div>
                    ...
                    <input
                      type="number" min={1} value={r.page}
                      onChange={e=>handleRotationChange(idx,'page',+e.target.value)}
                      className="mt-1 w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                    />
                  </div>
                  <div>
                    <label className="block text-sm text-gray-600">Degrees</label>
                    <input
                      type="number" value={r.degrees}
                      onChange={e=>handleRotationChange(idx,'degrees',+e.target.value)}
                      className="mt-1 w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                    />
                  </div>
                  <button type="button" onClick={()=>removeRotation(idx)}
          ...
          >{loading ? 'Rotating...' : 'Rotate PDF'}</button>

          {downloadUrl && (
            <a href={downloadUrl} download="rotated.pdf"
               className="block text-center text-blue-600 hover:underline mt-4"
            >📥 Download Rotated PDF</a>
          )}
        </form>
      </fieldset>
    </div>
